...
async def selecionar_marca_por_nome(page, nome_marca: str, tentativas=3) -> bool:
    for tentativa in range(1, tentativas + 1):
        try:
            ...
            marca_input = "#selectMarcacarro_chosen .chosen-search input"
            await page.fill(marca_input, nome_marca)
            await pausa_curta()

            itens = await page.query_selector_all("#selectMarcacarro_chosen ul.chosen-results li")
            clicou = False
            for li in itens:
            ...
            sel = (await page.locator("#selectMarcacarro_chosen span").inner_text()).strip()
...
async def coletar_modelos_da_marca(page) -> list[str]:
    ...
    lis = await page.query_selector_all("#selectAnoModelocarro_chosen ul.chosen-results > li")
    modelos = []
    for li in lis:
        txt = (await li.inner_text()).strip()
...
async def scan_modelos_por_marca(caminho_excel_marcas="marcas_Carros.xlsx", saida_json="catalogo_modelos_carros.json", saida_excel="catalogo_modelos_carros.xlsx"):
    ...
    async with async_playwright() as p:
        ...
        for i, marca in enumerate(marcas, start=1):
            ...
            try:
                ...
                try:
                    await page.click('#buttonLimparPesquisarcarro a.text', timeout=5000)
                    await pausa_media()
