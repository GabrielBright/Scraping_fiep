...
if not os.path.exists("marcas_processadas_caminhoes.json"):
...
async def run(max_marcas=None, max_modelos=None, max_anos=None):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            logging.info("Clicando em 'Consulta de Caminhões e Micro-Ônibus'...")
            await page.wait_for_selector('li:has-text("Caminhões e Micro-Ônibus")', state='visible', timeout=30000)
                        ...
                        try:
                            ...
                            for ano_index in range(max_anos_loop):
                                try:
                                    ...
                                    pd.DataFrame(Fipe).to_excel("Fipe_temp_caminhao.xlsx", index=False)
